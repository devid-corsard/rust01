...
pub fn decoder(inp: &str) -> u32 {
    ...
    let res: u32 = inp
        ...
        .map(|line| {
            let line = line
                .split(" | ")
                .map(|half| half.split_whitespace().collect::<Vec<&str>>())
                .collect::<Vec<Vec<&str>>>();
...
pub fn decoder_fail(inp: &str) -> u32 {
    let inp: u32 = inp
        .lines()
        .map(|line| {
            let line = line
                ...
                .collect::<Vec<Vec<String>>>();
            let mut map = [""; 10];
            line[0].iter().for_each(|codnum| match codnum.len() {
                2 => map[1] = codnum,
                3 => map[7] = codnum,
                4 => map[4] = codnum,
                7 => map[8] = codnum,
            ...
            line[0].iter().for_each(|codnum| match codnum.len() {
                5 => {
                    println!("cn {} m1 {}", codnum, map[1]);
                    if codnum.contains(map[1]) {
                        map[3] = codnum;
                    }
                }
                6 => {
                    if codnum.contains(map[7]) {
                        if codnum.contains(map[4]) {
                            map[9] = codnum;
                        } else {
                            map[0] = codnum;
                        }
            ...
            println!("{:?}", map);
            String::from_iter(line[1].iter().map(|abracadabra| {
                map.iter()
                    .position(|&x| x == abracadabra)
                    .unwrap()
                    .to_string()
            }))
            .parse::<u32>()
